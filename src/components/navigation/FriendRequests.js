	...
	const addFriend = async friend => {
		...
		try {
			...
			if (user1.username === user.username) {
				...
				if (!res3.ok) {
					throw new Error('Problem adding friend')
				}
				...
				if (!res4.ok) {
					throw new Error('Problem adding self to friendlist of friend')
				}
	...
	return (
		<>
			<Dropdown className='dropdown'>
				<Dropdown.Toggle variant='success' className='messageToggle' id='dropdown-basic'>
					<img src={notificationIcon} alt='Message Icon' className='hasSound' />
				</Dropdown.Toggle>
						...
						? friendRequests.map(request => {
								return (
									<div key={request} className='dropdownItem'>
										<div className='text'>
											Friend request from <h3 className='requesterName'>{request}</h3>
										</div>
										<div className='acceptReject'>
											<button onClick={addFriendStart} id={request} className='acceptInvite'>
												Accept
											</button>
											<button onClick={onRemoveRequest} id={request} className='rejectInvite'>
												Reject
											</button>
										</div>
									</div>
