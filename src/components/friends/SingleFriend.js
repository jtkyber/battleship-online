...
const SingleFriend = ({ friendInGame, name, status }) => {
	...
	const sendInvite = async e => {
		...
		try {
			...
			if (user1.username) {
				await fetch(
					`${process.env.REACT_APP_PUSHER_URL}/sendInvite?username=${user.username}&friendName=${user1.username}`
				)
	...
	const acceptInvite = async e => {
		await fetch(
			`${process.env.REACT_APP_PUSHER_URL}/sendGoToGame?username=${user.username}&friendName=${name}`
		)
		...
		<div id={name} value={name} className='friendBlock'>
			<div className='friendText'>
				...
				{friendInGame && status === 'online' ? (
					<button disabled className={`btn${name} friendInGameBtn`}>
						In Game
					</button>
				) : status === 'online' ? (
					<button className={`btn${name} friendOnlineBtn`} onClick={handleOnClick} id={name}>
						Invite
					</button>
				) : (
					<button disabled className={`btn${name} friendOfflineBtn`}>
						Offline
					</button>
				)}
			</div>
