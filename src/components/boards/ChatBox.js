...
const ChatBox = () => {
	const { opponentName, chatText, showChatMobile, isMobile, playingWithAI, channel } = useStoreState(
		state => ({
	...
	const handleEnterBtn = async e => {
		if (e.key === 'Enter' && chatText !== '') {
			...
			setChatText('')
			await fetch(
				`${process.env.REACT_APP_PUSHER_URL}/sendMessage?channelName=${opponentName}&message=${chatText}`
			)
	...
	return (
		<div
			className={`${!showChatMobile && isMobile ? 'hide' : null} ${
				isMobile ? 'chatContainerMobile' : 'chatContainer'
			}`}>
