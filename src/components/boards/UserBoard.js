...
const UserBoard = () => {
	...
	const getChoicesForNearby = hit => {
		const possibleChoices = [
			!aiShipOrientationGuess || aiShipOrientationGuess === 'hor'
				? `${parseInt(colString(hit)) - 1}-${parseInt(rowString(hit))}`
				: null,
			!aiShipOrientationGuess || aiShipOrientationGuess === 'hor'
				? `${parseInt(colString(hit)) + 1}-${parseInt(rowString(hit))}`
				: null,
			!aiShipOrientationGuess || aiShipOrientationGuess === 'vert'
				? `${parseInt(colString(hit))}-${parseInt(rowString(hit)) - 1}`
				: null,
			!aiShipOrientationGuess || aiShipOrientationGuess === 'vert'
				? `${parseInt(colString(hit))}-${parseInt(rowString(hit)) + 1}`
				: null,
			...
			for (let square of allSquareIDs) {
				if (square === choice) {
					...
					aiShipOrientationGuess
						? (aiMoveText = `AI: Choosing ${
								aiShipOrientationGuess === 'hor' ? 'horizontal' : 'vertical'
						  } neighbor to last hit --> `)
		...
		if (numOfValidSquares <= 0) {
			...
			for (let choice of possibleChoices) {
				for (let square of allSquareIDs) {
					if (square === choice) {
						foundOption = true
						aiMoveText = `AI: Going back to first hit with another ${
							aiShipOrientationGuess === 'hor' ? 'horizontal' : 'vertical'
						} guess --> `
					}
				}
	...
	const applyHitOrMiss = async oppShot => {
		if (matchOppShotToBoard(oppShot)) {
			...
			if (!playingWithAI)
				await fetch(
					`${process.env.REACT_APP_PUSHER_URL}/sendResultToOpponentBoard?channelName=${opponentName}&shotSquare=${oppShot.id}&shot=hit&shipHit=${shipHit}`
				)
		} else {
			...
			if (!playingWithAI)
				await fetch(
					`${process.env.REACT_APP_PUSHER_URL}/sendResultToOpponentBoard?channelName=${opponentName}&shotSquare=${oppShot.id}&shot=miss&shipHit=${shipHit}`
				)
		}
	}
